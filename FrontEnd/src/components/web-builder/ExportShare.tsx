...
import { Download, Share2, Mail, Link, FileImage, FileText, Users, Lock } from 'lucide-react';
...
export function ExportShare({ charts, filters }: ExportShareProps) {
  const [exportFormat, setExportFormat] = useState<'pdf' | 'png' | 'excel' | 'json'>('pdf');
  ...
  const handleExport = async (format: typeof exportFormat) => {
    ...
    setTimeout(() => {
      ...
      switch (format) {
        case 'json':
          content = JSON.stringify({ charts, filters, metadata: { name: dashboardName, exportedAt: new Date() } }, null, 2);
          mimeType = 'application/json';
          break;
        case 'excel':
          content = 'Dashboard,Charts,Filters\n' + charts.map(c => `${c.title},${c.type},${c.data.length} rows`).join('\n');
          mimeType = 'text/csv';
          break;
        default:
  ...
  const generateShareLink = () => {
    const shareUrl = `https://dataviz-pro.com/dashboard/${dashboardName.toLowerCase().replace(/\s+/g, '-')}-${Date.now()}`;
    navigator.clipboard.writeText(shareUrl);
    toast.success('Share link copied to clipboard');
  };

  const exportOptions = [
    { value: 'pdf', label: 'PDF Document', icon: FileText, description: 'High-quality PDF with all charts' },
    { value: 'png', label: 'PNG Images', icon: FileImage, description: 'Individual chart images' },
    { value: 'excel', label: 'Excel Workbook', icon: FileText, description: 'Data tables and charts' },
    { value: 'json', label: 'JSON Data', icon: FileText, description: 'Raw data and configuration' }
  ];
        ...
        <TabsContent value="export" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Download your dashboard in various formats for offline use or presentation
              </CardDescription>
            </CardHeader>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {exportOptions.map((option) => (
                    <Card 
                      ...
                      onClick={() => setExportFormat(option.value as typeof exportFormat)}
                    >
              ...
              <div className="flex items-center justify-between p-4 bg-muted rounded-lg">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {charts.length} charts • {filters.length} filters • {exportFormat.toUpperCase()} format
                  </p>
                </div>
                <Button onClick={() => handleExport(exportFormat)} disabled={charts.length === 0}>
              ...
              {isPublicDashboard && (
                <div className="space-y-4 p-4 bg-muted rounded-lg">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <Input
                        value={`https://dataviz-pro.com/public/${dashboardName.toLowerCase().replace(/\s+/g, '-')}`}
                        readOnly
                      />
                      <Button variant="outline" onClick={generateShareLink}>
                  ...
                  <div key={dashboard.id} className="border rounded-lg p-4">
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm">Shared with {dashboard.sharedWith.length} people</span>
