...
export function SQLBuilder({ showAIAssistant, setShowAIAssistant }: SQLBuilderProps) {
  const { setAnalyzedData, dataSources } = useData();
  const [activeQuery, setActiveQuery] = useState('');
  const [queryResults, setQueryResults] = useState<any[]>([]);
  const [isExecuting, setIsExecuting] = useState(false);
  const [activeTab, setActiveTab] = useState<'editor' | 'import' | 'saved'>('editor');
  ...
  const handleExecuteQuery = async (query: string) => {
    ...
    try {
      // Execute query using backend API
      const result = await executeQuery(query);
      
      setQueryResults(result.data);
      
      // Update analyzed data context
      ...
      toast.success(`Query executed successfully. ${result.rows_returned} rows returned.`);
    } catch (error) {
  ...
  const inferColumnType = (columnName: string, data: any[]): 'string' | 'number' | 'date' => {
    if (data.length === 0) return 'string';
    
    const firstValue = data[0][columnName];
    const lowerName = columnName.toLowerCase();
    
    if (lowerName.includes('date') || lowerName.includes('time') || lowerName.includes('_at')) {
      return 'date';
    }
  ...
  const generateColumnsFromResults = (results: any[]) => {
    ...
    return Object.keys(results[0]).map(key => ({
      name: key,
      type: typeof results[0][key] === 'number' ? 'number' as const : 
            key.includes('date') || key.includes('_at') ? 'date' as const : 'string' as const,
      originalName: key
    }));
  };
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'import':
        return <DataImport />;
      case 'saved':
        return <SavedQueries onLoadQuery={setActiveQuery} />;
      default:
        return (
